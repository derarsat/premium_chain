            ...
            <tr class="bg-white border-b">
                ...
                <td class="px-6 py-4 flex gap-3">
                    <button
                        ...
                        data-modal-target="country-edit-modal"
                        data-modal-toggle="country-edit-modal"
                    ...
                    <button
                        data-modal-target="country-delete-modal"
                        data-modal-toggle="country-delete-modal"
                        class="font-medium text-red-600 hover:underline flex gap-1 items-center"
                        onclick="handleCountryDelete(event)"
                        data-country-id="{{$country->id}}"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            <button
                ...
                data-modal-hide="country-edit-modal"
            >
                <svg
                    class="w-3 h-3"
                    aria-hidden="true"
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form
                    method="post"
                    class="space-y-6"
                    action="{{ route('country-controller') }}?action=update"
                >
                    @csrf
    ...
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow">
            <button
                type="button"
                class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center"
                data-modal-hide="country-delete-modal"
            >
                <svg
                    class="w-3 h-3"
                    aria-hidden="true"
            ...
            <div class="p-6 text-center">
                ...
                <form method="post" class="inline-block" action="{{ route('country-controller') }}?action=delete">
...
<script>
    function handleCountryClick() {
        ...
        const name = event.target.getAttribute("data-country-name");
        inputId.value = event.target.getAttribute("data-country-id");
        inputName.value = name;
    }

    function handleCountryDelete() {
        const inputId = document.getElementById("delete-country-id");
        inputId.value = event.target.getAttribute("data-country-id");
    }
</script>
